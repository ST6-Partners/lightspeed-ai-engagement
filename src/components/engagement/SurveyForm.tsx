// Take Survey — the engagement survey form (moved under the Engagement Survey
// page's "Take Survey" tab). 5 Likert sections (66 statements) + eNPS 0..10 +
// confidential open text. Submits via engagementSurvey.submit.
...
export default function SurveyForm() {
  ...
  const complete = answeredCount === LIKERT_COUNT && enpsScore != null;
  const set = (id: string, v: number) => setAnswers((p) => ({ ...p, [id]: v }));
  const missing = (id: string) => showGaps && typeof answers[id] !== 'number';

  const handleSubmit = () => {
    if (!complete) { setShowGaps(true); document.querySelector('[data-missing="1"]')?.scrollIntoView({ behavior: 'smooth', block: 'center' }); return; }
    ...
    return (
      <div className="ls-card p-8 text-center border-l-4 border-ls-thrive">
        ...
        <p className="text-sm text-ls-ink-2 max-w-lg mx-auto">
          Your answers are confidential and feed the aggregate engagement read on the other tabs.
        </p>
      </div>
    );
  }
  ...
  return (
    <div>
      <p className="text-sm text-ls-ink-3 mb-4">
        Rate how strongly you agree or disagree with each statement. Takes ~6 minutes.
      </p>
      <div className="ls-card p-3 mb-5 text-[13px] text-ls-ink-2">
        🔒 <b>Confidential.</b> Your individual responses are kept confidential and used only in aggregate.
      </div>
      ...
      <div className="flex items-center gap-3 mt-6 mb-10">
        ...
        {!complete && showGaps && <span className="ls-chip bg-ls-watch-bg text-ls-watch">Please answer every required question</span>}
