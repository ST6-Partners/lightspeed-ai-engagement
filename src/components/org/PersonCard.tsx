...
export default function PersonCard({ person, tab, periodId, reviewPeriod, okrPeriodId, cadence, readOnly, prioritiesPeriodKey }: { person: Person; tab: TabKey; periodId?: string; reviewPeriod?: string | null; okrPeriodId?: string; cadence?: 'done' | 'due' | 'overdue'; readOnly?: boolean; prioritiesPeriodKey?: string; managerName?: string | null; reportingLine?: string | null }) {
  ...
  return (
    <div className="rounded-lg p-5" style={{ background: TOKENS.panel, border: `1px solid ${TOKENS.borderSoft}` }}>
      ...
      <div style={{ borderTop: `1px solid ${TOKENS.borderSoft}`, paddingTop: 14 }}>
        {tab === 'priorities' && <PrioritiesTab employeeId={person.id} readOnly={readOnly} periodKey={prioritiesPeriodKey} />}
