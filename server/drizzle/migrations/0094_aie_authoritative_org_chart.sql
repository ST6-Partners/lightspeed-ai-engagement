-- AIE 2026-07-28 — AUTHORITATIVE org-chart load from the 07-22 Advanced Report
-- Writer export (225 employees). Sets each person's PRIMARY manager by matching
-- the manager's HR employee code (from Reports_to_Position) to users.external_id
-- (populated in 0025) — name-independent, so nicknames/aliases (Robert/Carson,
-- 'Steven Miller Junior') and hyphenated names resolve correctly. Employee is
-- targeted by name (from the same legal-name import). Roots: Brian Thomas (CEO)
-- and two seats reporting to a VACANT position (Matthew Burg, Gregory Artzt) -> NULL.
-- Idempotent; safe to re-run. Then re-syncs user_managers to mirror the single manager.
...
--> statement-breakpoint
UPDATE "users" SET "manager_id"=NULL, "updated_at"=now() WHERE lower("name")=lower('Matthew Burg');
--> statement-breakpoint
...
--> statement-breakpoint
UPDATE "users" SET "manager_id"=NULL, "updated_at"=now() WHERE lower("name")=lower('Gregory Artzt');
--> statement-breakpoint
